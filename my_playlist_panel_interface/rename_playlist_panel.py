...
class RenamePlaylistPanel(SubPanelFrame):
    ...
    def __init__(self, oldPlaylist: dict, parent=None):
        ...
        # 实例化子属性面板
        self.__subRenamePlaylistPanel = SubRenamePlaylistPanel(
            oldPlaylist, self)
...
class SubRenamePlaylistPanel(BasicPlaylistPanel):
    ...
    def __lineEditTextChangedSlot(self, playlistName: str):
        ...
        # 如果播放列表名已存在，显示提示标签
        if self._isPlaylistExist(playlistName) and self.height() != 627 and playlistName != self.oldPlaylistName:
            self.resize(586, 627)
            self.resizeSig.emit()
            self.__initLayout()
    ...
    def __renamePlaylistButtonSlot(self):
        ...
        # 重命名文件
        rename(f'Playlists\\{self.oldPlaylistName}.json',
