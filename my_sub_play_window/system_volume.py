...
class SystemVolume():
    ...
    def setVolume(self, volume):
        """ 设置系统音量 """
        if 0 <= volume <= 100:
            self.__volume.SetMasterVolumeLevel(
                self.volumeDBTable[str(volume)], None)
        else:
    ...
    def __getVolumeDBTable(self):
        """ 读取音量与分贝的对应表 """
        with open(r'resource\table\volume_db_table.json', 'r') as f:
            self.volumeDBTable = load(f)  # type:dict

    def getVolumeTableManually(self):
        """ 手动调节音量来获取完整的音量-分贝表 """
        db_set = set()
        with open(r'resource\table\volume_db_table.json', 'w') as f:
            print('正在读取音量...')
