...
class ToolButton(NavigationButton):
    ...
    def __init__(self, icon_path, parent=None, buttonSize=(60,60)):
        super().__init__(icon_path, text='', parent=parent, buttonSize=buttonSize, iconSize=buttonSize)
    ...
    def paintEvent(self, e):
        ...
        if self.isSelected == True:
            ...
            if not self.pressedPos:
                ...
                # 绘制选中标志
                self.drawLine(painter, 2, 2, 6, 2, 4,
                              self.height() - 2, 0, self.height() - 2)
            elif self.pressedPos in ['left-bottom', 'right-top', 'bottom']:
                self.drawLine(painter, 0, 1, 4, 1, 6,
                              self.height() - 2, 2, self.height() - 2)
            elif self.pressedPos in ['left', 'right', 'center']:
                self.drawLine(painter, 1, 2, 5, 2, 5,
        ...
        if not self.pressedPos:
            ...
            image = self.image.scaled(self.image.width() - 4, self.image.height() - 4,
...
class PushButton(NavigationButton):
    """ 显示图标和文字的按钮 """
    def __init__(self, icon_path, text='', parent=None, buttonSize=(60,60), iconSize=(60,60)):
    ...
    def paintEvent(self, e):
        ...
        if self.isSelected == True:
            ...
            # 左上角和顶部
                ...
                self.drawLine(painter, 5, 2, 9, 2, 7,
                              self.height() - 2, 3, self.height() - 2)
            # 左下角和底部
            elif self.pressedPos in ['left-bottom', 'bottom']:
                self.drawLine(painter, 3, 2, 7, 2, 9,
                              self.height() - 3, 5, self.height() - 3)
            # 左边和中间
            elif self.pressedPos in ['left', 'center']:
                self.drawLine(painter, 5, 2, 9, 2, 9,
                              self.height() - 2, 5, self.height() - 2)
            # 右上角
            elif self.pressedPos == 'right-top':
                self.drawLine(painter, 0, 2, 4, 2, 3, self.height() -
                              2, 0, self.height() - 2)
            # 右边
            elif self.pressedPos == 'right':
                self.drawLine(painter, 1, 1, 5, 1, 5, self.height() -
                              1, 1, self.height() - 1)
            # 右下角
            elif self.pressedPos == 'right-bottom':
                self.drawLine(painter, 0, 2, 3, 2, 4,
                              self.height()-2, 0, self.height() - 2)
        ...
        if not self.pressedPos:
            ...
            image = self.image.scaled(self.image.width() - 2, self.image.height() - 2,
                                      Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def drawTextIcon(self, painter, image: QPixmap, shearX: float=0, shearY: float=0, iconX=0, iconY=0, textX=60, textY=21,fontSize=11):
        """ 绘制图标和文字 """
        painter.shear(shearX, shearY)
        # 绘制图标
