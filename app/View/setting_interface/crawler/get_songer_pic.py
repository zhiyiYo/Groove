...
class SongerInfo:
    ...
    def getSongerInfo(self):
        """ 开始爬取歌手信息 """
        # 从json文件中读取歌手信息
        with open(r"app\data\songerInfo.json", encoding="utf-8") as f:
            try:
        ...
        for info_dict in self.songInfo.songInfo_list:
            print(
                f"\r当前进度:{self.songInfo.songInfo_list.index(info_dict)/len(self.songInfo.songInfo_list):%}",
                end="",
                flush=True,
            )
        ...
        # 更新json文件
        with open("app\\data\\songerInfo.json", "w", encoding="utf-8") as f:
    ...
    def crawlInfo(self, info_dict, url):
        ...
        try:
            songer_page_link = WebDriverWait(self.browser, 3).until(
