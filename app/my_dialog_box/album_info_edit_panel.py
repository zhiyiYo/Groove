...
from app.my_functions.get_pic_suffix import getPicSuffix
from app.my_widget.my_label import ErrorIcon
from app.my_widget.my_lineEdit import LineEdit
from app.my_widget.my_scrollArea import ScrollArea
...
class AlbumInfoEditPanel(SubPanelFrame):
    ...
    def initWidget(self):
        """ 初始化小部件 """
        self.showMask()
        self.saveInfoSig = self.subAlbumInfoEditPanel.saveInfoSig
        self.saveErrorSlot = self.subAlbumInfoEditPanel.saveErrorSlot
        self.saveCompleteSlot = self.subAlbumInfoEditPanel.saveCompleteSlot
        self.subAlbumInfoEditPanel.adjustHeightSig.connect(self.initLayout)

    def initLayout(self):
        """ 初始化布局 """
        self.subAlbumInfoEditPanel.move(
            int(self.width() / 2 - self.subAlbumInfoEditPanel.width() / 2),
