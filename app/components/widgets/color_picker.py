...
class SliderWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.brightnessSlider = CSlider(Qt.Horizontal)
        self.saturationSlider = CSlider(Qt.Horizontal)
...
class ColorCircle(QWidget):
    ...
    def __init__(self, startColor=QColor(0, 255, 255), parent=None):
        ...
        self.hueCircle.setPixmap(QPixmap(
            ':/images/color_picker/wheel.png').scaled(250, 250, Qt.KeepAspectRatio, Qt.SmoothTransformation))
...
class ColorPickerPanel(QWidget):
    ...
    def __init__(self, startColor=QColor(255, 0, 0), parent=None):
        ...
        # connect signal to slot
        self.colorCircle.colorChanged.connect(self.sliderWidget.setColor)
        ...
        self.yesButton.clicked.connect(
            lambda: self.colorChanged.emit(self.colorCircle.color))

        self.sliderWidget.setColor(startColor)

    def setColor(self, color: QColor):
        color = QColor(color)
        self.colorCircle.setColor(color)
...
class ColorPicker(QFrame):
    ...
    def __init__(self, color: list, parent=None):
        ...
        self.setAttribute(Qt.WA_TranslucentBackground)
        setStyleSheet(self, 'color_picker')

        self.color = QColor(color)
        self.colorPickerPanel = ColorPickerPanel(QColor(color), self)
        self.colorPickerPanel.colorChanged.connect(self.__onColorChanged)
        self.colorPickerPanel.hide()
        self.setCursor(Qt.PointingHandCursor)

        self.shadowEffect = QGraphicsDropShadowEffect(self)
        c = 255 if config.theme == "dark" else 0
        self.shadowEffect.setColor(QColor(c, c, c, 100))
        self.shadowEffect.setOffset(0, 1)
        self.shadowEffect.setBlurRadius(4)
    ...
    def mousePressEvent(self, e):
        ...
        self.colorPickerPanel.setColor(self.color)
