...
from app.crawler import KuGouCrawler, QQMusicCrawler
from mutagen import File
from mutagen.id3 import TIT2, TPE1, TPE2
...
class GetMetaDataThread(QThread):
    ...
    def __init__(self, targetPath_list=None, parent=None):
        ...
        # 存放专辑封面的文件夹
        self.albumCoverFolder = r"app\resource\crawl_albums"
        # 拆分音频文件路径
        self.splitText()

    def run(self):
        """ 开始爬取信息 """
        self.kuGouCrawler = KuGouCrawler(self.albumCover_set, self.albumCoverFolder)
        for index, (songer, songname, songPath) in enumerate(
            zip(self.songer_list, self.songname_list, self.songPath_list)
        ):
        ...
        for index, (songname, songPath) in enumerate(
            zip(self.songname_list, self.songPath_list)
        ):
            if not self.keepRunning:
    ...
    def modifyTextPart(self, songname, songer) -> bool:
        ...
        if suffix == "mp3":
            # 如果没有标题则添加标题
            if (
                not self.id_card.get("TIT2")
                or str(self.id_card.get("TIT2")) != songname
            ):
            ...
            if not self.id_card.get("TPE1") or str(self.id_card.get("TPE1")) != songer:
                ...
                isModified = True
            if not self.id_card.get("TPE2") or str(self.id_card.get("TPE2")) != songer:
                self.id_card["TPE2"] = TPE2(encoding=3, text=songer)
                isModified = True
        elif suffix == "flac":
            if (
                not self.id_card.get("title")
                or self.id_card.get("title")[0] != songname
            ):
                self.id_card["title"] = songname
                isModified = True
            # 如果没有歌手名则添加歌手名
            if (
                not self.id_card.get("artist")
                or self.id_card.get("artist")[0] != songer
            ):
                self.id_card["artist"] = songer
                isModified = True
        elif suffix == "mp4":
            # 如果没有标题则添加标题
            if not self.id_card.get("©nam") or self.id_card["©nam"][0] != songname:
    ...
    def fetchAlbum(self, songer, songname) -> bool:
        ...
        if suffix == "mp3":
            ...
            # 专辑提取条件
            album_get_cond = (
                not self.id_card.get("TDRC")
                or not self.id_card.get("TALB")
                or str(self.id_card.get("TDRC"))[0] == "0"
                or (suffix == "mp3" and not Match)
            )
            ...
            flac_write_cond = not self.id_card.pictures or (
                not self.id_card.get("year")
                or not self.id_card.get("album")
            ...
            album_get_cond = (
                not self.id_card.get("covr")
                or not self.id_card.get("©alb")
                or not self.id_card.get("©day")
                or self.id_card.get("©day")[0][0] == "0"
            )
            if album_get_cond:
                self.kuGouCrawler.get_album(songer, songname, self.id_card)
        return isModified
