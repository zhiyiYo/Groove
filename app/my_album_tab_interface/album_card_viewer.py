...
import pinyin
from app.my_widget.my_grid_layout import GridLayout
from app.my_widget.my_groupBox import GroupBox
...
from app.my_dialog_box.delete_card_panel import DeleteCardPanel
from app.my_object.save_info_object import SaveInfoObject
...
class AlbumCardViewer(QWidget):
    ...
    def __createOneAlbumCard(self, albumInfo: dict):
        ...
        self.albumSonger2AlbumCard_dict[
            albumInfo["album"] + "." + albumInfo["songer"]
        ] = albumCard
        # 专辑卡信号连接到槽函数
    ...
    def __connectGroupBoxSigToSlot(self, layout):
        ...
        for group_dict in self.currentGroupDict_list:
            group_dict["container"].titleClicked.connect(
                lambda: self.showLabelNavigationInterfaceSig.emit(
                    self.groupTitle_list, layout
                )
            )
